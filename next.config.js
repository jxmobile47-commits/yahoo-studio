...
const nextConfig = {
  ...
  // output: 'standalone', // Uncomment for Docker builds
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    if (!dev && !isServer) {
      ...
      config.output.devtoolModuleFilenameTemplate = function (info) {
        ...
        return `webpack://yahoo-studio/${rel}`;
      };
      config.output.devtoolFallbackModuleFilenameTemplate = function (info) {
        return `webpack://yahoo-studio/${info.resourcePath}?${info.hash}`;
      };
    } else {
  ...
  // Source map configuration - Security: only expose in production when explicitly enabled
