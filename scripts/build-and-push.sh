...
# Yahoo Studio Docker Build and Push Script (MANUAL USE ONLY)
...
if [[ -z "$REGISTRY_TYPE" ]]; then
    print_header "Yahoo Studio Docker Build and Push"
...
case $REGISTRY_TYPE in
    dockerhub)
        ...
        FRONTEND_IMAGE="$USERNAME/yahoo-studio-frontend:$VERSION_TAG"
        BACKEND_IMAGE="$USERNAME/yahoo-studio-backend:$VERSION_TAG"
    ...
    ghcr)
        ...
        FRONTEND_IMAGE="ghcr.io/$USERNAME/yahoo-studio/frontend:$VERSION_TAG"
        BACKEND_IMAGE="ghcr.io/$USERNAME/yahoo-studio/backend:$VERSION_TAG"
    ...
    gcr)
        ...
        FRONTEND_IMAGE="gcr.io/$PROJECT_ID/yahoo-studio-frontend:$VERSION_TAG"
        BACKEND_IMAGE="gcr.io/$PROJECT_ID/yahoo-studio-backend:$VERSION_TAG"
        REGISTRY_URL="gcr.io"
        ;;
    local)
        FRONTEND_IMAGE="yahoo-studio-frontend:$VERSION_TAG"
        BACKEND_IMAGE="yahoo-studio-backend:$VERSION_TAG"
        ;;
    *)
...
build_images() {
    ...
    docker images | grep -E "(yahoo-studio|$USERNAME)" | head -10
}
