...
describe('Chord Simplification', () => {
  describe('simplifyChord', () => {
    test('should simplify major chords correctly', () => {
      ...
      expect(simplifyChord('F#')).toBe('F#'); // Preserves original enharmonic spelling
      expect(simplifyChord('Bb:maj')).toBe('Bb'); // Preserves original enharmonic spelling
    });
    ...
    test('should simplify minor chords correctly', () => {
      ...
      expect(simplifyChord('F#m')).toBe('F#m'); // Preserves original enharmonic spelling
      expect(simplifyChord('Bbmin')).toBe('Bbm'); // Preserves original enharmonic spelling
    ...
    test('should simplify augmented chords correctly', () => {
      ...
      expect(simplifyChord('F#aug')).toBe('F#aug'); // Preserves original enharmonic spelling
      expect(simplifyChord('Bbaug7')).toBe('Bbaug'); // Preserves original enharmonic spelling
    });
    ...
    test('should simplify diminished chords correctly', () => {
      ...
      expect(simplifyChord('F#dim')).toBe('F#dim'); // Preserves original enharmonic spelling
    });
    ...
    test('should handle edge cases that reveal operator precedence bug', () => {
      ...
      expect(simplifyChord('Bbdim7')).toBe('Bbdim');

      // Test that actual minor chords work correctly
      expect(simplifyChord('Fm')).toBe('Fm');
      expect(simplifyChord('F#min')).toBe('F#m');
      expect(simplifyChord('Bbmin7')).toBe('Bbm');
    });

    test('should handle the specific Yahoo Studio bug case', () => {
  ...
  describe('simplifyChordCorrections', () => {
    ...
    test('should handle the Yahoo Studio conflict case', () => {
  ...
  describe('Edge cases', () => {
    ...
    test('should handle mixed case input', () => {
      expect(simplifyChord('cMAJ7')).toBe('cMAJ7'); // Lowercase root is not matched by [A-G] regex, passes through unchanged
