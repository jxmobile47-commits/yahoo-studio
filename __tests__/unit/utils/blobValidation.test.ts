...
describe('offloadValidation URL handling', () => {
  describe('validateOffloadUrl', () => {
    it('accepts a Firebase Storage download URL', () => {
      const url = 'https://firebasestorage.googleapis.com/v0/b/yahoo-studio.appspot.com/o/temp%2Ftest.mp3?alt=media&token=abc';

      expect(validateOffloadUrl(url)).toBe(url);
    });

    it('accepts a storage.googleapis.com object URL', () => {
      const url = 'https://storage.googleapis.com/yahoo-studio.appspot.com/temp/test.mp3';

      expect(validateOffloadUrl(url)).toBe(url);
    });

    it('rejects non-https URLs', () => {
      expect(() => validateOffloadUrl('http://storage.googleapis.com/yahoo-studio.appspot.com/temp/test.mp3'))
  ...
  describe('getOffloadStorageProvider', () => {
    it('detects Firebase provider', () => {
      expect(getOffloadStorageProvider('https://firebasestorage.googleapis.com/v0/b/yahoo-studio.appspot.com/o/temp%2Ftest.mp3?alt=media'))
        .toBe('firebase');
    });
  ...
  describe('parseFirebaseStorageObjectFromUrl', () => {
    it('parses firebasestorage.googleapis.com URLs', () => {
      const parsed = parseFirebaseStorageObjectFromUrl(
        'https://firebasestorage.googleapis.com/v0/b/yahoo-studio.appspot.com/o/temp%2F123-test.mp3?alt=media&token=abc',
      );

      expect(parsed).toEqual({
        bucket: 'yahoo-studio.appspot.com',
    ...
    it('parses storage.googleapis.com URLs', () => {
      const parsed = parseFirebaseStorageObjectFromUrl(
        'https://storage.googleapis.com/yahoo-studio.appspot.com/temp/123-test.mp3',
      );
    ...
    it('returns null for invalid Firebase URL shape', () => {
      const parsed = parseFirebaseStorageObjectFromUrl(
        'https://firebasestorage.googleapis.com/v0/b/yahoo-studio.appspot.com/o/',
      );
