const BEAT_GRID_DEBUG_LOCAL_STORAGE_KEY = 'yahooStudioBeatGridDebug';
const BEAT_GRID_DEBUG_VERBOSE_LOCAL_STORAGE_KEY = 'yahooStudioBeatGridDebugVerbose';

type DebugWindow = Window & {
  __YahooStudio_BEAT_GRID_DEBUG__?: boolean;
  __YahooStudio_BEAT_GRID_DEBUG_VERBOSE__?: boolean;
};
...
export function isBeatGridDebugEnabled(): boolean {
  ...
  try {
    ...
    if (debugWindow.__YahooStudio_BEAT_GRID_DEBUG__ === true) {
      return true;
    }
...
export function isBeatGridDebugVerboseEnabled(): boolean {
  ...
  try {
    const debugWindow = window as DebugWindow;
    if (debugWindow.__YahooStudio_BEAT_GRID_DEBUG_VERBOSE__ === true) {
      return true;
    }
...
export function beatGridDebugLog(scope: string, message: string, payload?: unknown): void {
  ...
  const prefix = `[YahooStudio][BeatGridDebug][${scope}] ${message}`;
