...
jest.mock('@/utils/modelFiltering', () => ({
  getSafeBeatModel: jest.fn((model) => (model === 'beat-transformer' || model === 'madmom' ? model : 'madmom')),
...
describe('useModelState Hook', () => {
  ...
  it('loads saved values from localStorage', () => {
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'yahooStudio_beat_detector') return 'madmom';
      if (key === 'yahooStudio_chord_detector') return 'btc-sl';
      return null;
    });
  ...
  it('ignores invalid values from localStorage', () => {
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'yahooStudio_beat_detector') return 'invalid-detector';
      if (key === 'yahooStudio_chord_detector') return 'invalid-chord';
      return null;
    });
  ...
  it('updates beat detector and persists to localStorage', () => {
    ...
    expect(result.current.beatDetector).toBe('madmom');
    expect(localStorageMock.setItem).toHaveBeenCalledWith('yahooStudio_beat_detector', 'madmom');
  });
  ...
  it('updates chord detector and persists to localStorage', () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('yahooStudio_chord_detector', 'btc-pl');
  });
