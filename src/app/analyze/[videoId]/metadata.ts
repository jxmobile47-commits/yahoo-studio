...
export async function generateAnalyzeMetadata(videoId: string): Promise<Metadata> {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://yahoo-studio.app';
  const pageUrl = `${baseUrl}/analyze/${videoId}`;
  
  // Generate title and description
  const title = videoInfo?.title 
    ? `${videoInfo.title} - Chord Analysis | Yahoo Studio`
    : `Music Analysis ${videoId} | Yahoo Studio`;
    
  const description = videoInfo?.title
    ? `AI-powered chord recognition and beat detection analysis of "${videoInfo.title}". Discover chord progressions, beats, and musical structure with Yahoo Studio's advanced music analysis tools.`
    : `AI-powered music analysis for video ${videoId}. Discover chord progressions, beats, and musical structure with Yahoo Studio's advanced analysis tools.`;
