  ...
  it('uses forwarded proxy headers instead of an internal 0.0.0.0 origin', async () => {
    ...
    const response = await POST(makeRequest(
      ...
      'https://0.0.0.0:8080',
      {
        ...
        'x-forwarded-host': 'yahoo-studio-production.up.railway.app',
        host: '0.0.0.0:8080',
      },
    ));
    ...
    expect(mockBuildSegmentationRequestHash).toHaveBeenCalledWith(
      songContext,
      'https://yahoo-studio-production.up.railway.app/audio/generated/song.mp3',
    );
    expect(mockEnqueueSongFormerSegmentationTask).toHaveBeenCalledWith(expect.objectContaining({
      audioUrl: 'https://yahoo-studio-production.up.railway.app/audio/generated/song.mp3',
      callbackUrl: 'https://yahoo-studio-production.up.railway.app/api/segmentation/jobs/job-123',
    }));
  });
