...
import { getChordGridData } from '@/services/chord-analysis/chordGridCalculationService';
...
describe('Analyze Page Core Functionality', () => {
  describe('Chord Grid Data Processing', () => {
    const mockAnalysisResults = {
      ...
      downbeats: [0, 4, 8],
      downbeats_with_measures: [0, 4, 8],
    };

    it('processes analysis results correctly', () => {
      const result = getChordGridData(mockAnalysisResults as any);
    ...
    it('handles empty analysis results', () => {
      const emptyResults = {
        ...
        chordModel: 'chord-cnn-lstm',
        audioDuration: 0,
        beatDetectionResult: { time_signature: 4, bpm: 120, beatShift: 0 },
      };

      const result = getChordGridData(emptyResults as any);
      expect(result.chords).toEqual([]);
    });

    it('handles missing analysis results', () => {
      const result = getChordGridData(null as any);
      // The service returns an empty grid object instead of null
      expect(result.chords).toEqual([]);
      expect(result.beats).toEqual([]);
    });

    it('adapts to different time signatures', () => {
      // Create mock results with different time signatures
      const mockResults3_4 = {
        ...mockAnalysisResults,
        beatDetectionResult: { ...mockAnalysisResults.beatDetectionResult, time_signature: 3 }
      };
      const mockResults4_4 = {
        ...mockAnalysisResults,
        beatDetectionResult: { ...mockAnalysisResults.beatDetectionResult, time_signature: 4 }
      };

      const result3_4 = getChordGridData(mockResults3_4 as any);
      const result4_4 = getChordGridData(mockResults4_4 as any);
  ...
  describe('Beat Tracking Algorithms', () => {
    ...
    function findCurrentBeatIndex(currentTime: number, beats: Array<{time: number}> | null, lastBeatIndex: number = -1): number {
  ...
  describe('Model Selection and Persistence', () => {
    ...
    function getStoredBeatDetector(): string {
      const saved = mockLocalStorage.getItem('yahooStudio_beat_detector');
      if (saved && ['auto', 'madmom', 'beat-transformer'].includes(saved)) {
        return saved;
      }
      return 'madmom'; // default
    }

    function setStoredBeatDetector(value: string): void {
      mockLocalStorage.setItem('yahooStudio_beat_detector', value);
    }
    ...
    it('returns stored beat detector when valid', () => {
      mockLocalStorage.setItem('yahooStudio_beat_detector', 'madmom');
      expect(getStoredBeatDetector()).toBe('madmom');
    });

    it('returns default when stored value is invalid', () => {
      mockLocalStorage.setItem('yahooStudio_beat_detector', 'invalid-model');
      expect(getStoredBeatDetector()).toBe('madmom');
    });

    it('persists beat detector selection', () => {
      setStoredBeatDetector('auto');
      expect(mockLocalStorage.setItem).toHaveBeenCalledWith('yahooStudio_beat_detector', 'auto');
    });
  });
