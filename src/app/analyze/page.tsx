...
import ChordVariationPanel from '@/components/analysis/ChordVariationPanel';
...
export default function LocalAudioAnalyzePage() {
  ...
  const processAudioFile = async () => {
    ...
    try {
      ...
      // Map backend/offline errors to user-friendly messages
      const lower = rawError.toLowerCase();
      let errorMessage: string;

      // Backend offline detection — check these first
      const isBackendOffline =
        lower.includes('fetch failed') ||
        lower.includes('failed to fetch') ||
        lower.includes('networkerror') ||
        lower.includes('err_connection_refused') ||
        lower.includes('ensure python backend') ||
        lower.includes('backend returned') ||
        lower.includes('backend is running') ||
        lower.includes('backend unavailable') ||
        lower.includes('port conflict') ||
        lower.includes('python backend') ||
        lower.includes('backend service') ||
      ...
      if (isBackendOffline) {
        errorMessage = 'AI analysis backend is offline. Please start the Python backend server or try again later.';
      } else if (lower.includes('rate limited')) {
        errorMessage = 'Server is busy. Please wait a moment and try again.';
      } else if (lower.includes('too large') || lower.includes('413')) {
        errorMessage = 'Audio file is too large. Please use a shorter clip (under 10 minutes).';
      } else if (lower.includes('timeout')) {
        errorMessage = 'Analysis timed out. Please try again with a shorter audio file.';
      } else if (lower.includes('no beats detected')) {
        errorMessage = 'No beats detected. Try the "madmom" detector or a different audio file.';
      } else if (lower.includes('no chords detected')) {
        errorMessage = 'No chords detected. Please try again with a different audio file.';
      } else {
        errorMessage = rawError || 'An unexpected error occurred. Please try again.';
      }
  ...
  // Memoize chord data preparation for key detection (expensive filter/map/dedup)
  const preparedChordData = useMemo(() => {
    if (!analysisResults?.chords?.length) return { chordData: [], chordNames: [], heuristicKey: '' as string };

    const rawChordData = analysisResults.chords
      .filter((chord) => chord.time !== undefined && chord.time !== null)
      .map((chord) => ({ chord: chord.chord, time: chord.time as number }));

    const chordData = rawChordData.filter((entry, index) => {
      if (index === 0) return true;
      return entry.chord !== rawChordData[index - 1].chord;
    });

    const chordNames = chordData.map((entry) => entry.chord);
    const heuristicKey = estimateKeySignatureFromChords(chordNames).keySignature;

    return { chordData, chordNames, heuristicKey };
  }, [analysisResults?.chords]);

  // Key detection effect - triggers once per unique chord set
  useEffect(() => {
    const { chordData, heuristicKey } = preparedChordData;
    if (chordData.length > 0 && !isDetectingKey && !keyDetectionAttempted) {
      setIsDetectingKey(true);
      setKeyDetectionAttempted(true);
      setKeySignature(heuristicKey);
  ...
  }, [preparedChordData, isDetectingKey, keyDetectionAttempted]);
                      ...
                      <ScrollableTabContainer variant="plain" heightClass="h-[60vh] md:h-[66vh]">
                        <div className={`flex flex-col md:flex-row gap-4`}>
                          ...
                          <div className={`w-full transition-all duration-200`}>
                            ...
                            />
                            <ChordVariationPanel chords={simplifiedChordGridData?.chords || []} />
                            <div className="mt-3">
                              <BeatTimeline
                                beats={analysisResults?.beats || []}
                                downbeats={analysisResults?.downbeats || []}
