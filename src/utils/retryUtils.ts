/**
 * Retry Utilities for Yahoo Studio
 * 
...
import { safeFetchAudioSource } from './safeServerAudioFetch';
...
export const RETRY_STRATEGIES: RetryStrategy[] = [
  ...
  },
  {
    ...
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; Yahoo Studio-CacheBuster/1.0)',
  ...
  },
  {
    ...
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; Yahoo Studio-URLBuster/1.0)',
      'Cache-Control': 'no-cache',
    },
...
export async function retryFetch(
  ...
  for (let attempt = 0; attempt < maxAttempts && attempt < RETRY_STRATEGIES.length; attempt++) {
    ...
    try {
      ...
      const response = await safeFetchAudioSource(requestUrl, {
