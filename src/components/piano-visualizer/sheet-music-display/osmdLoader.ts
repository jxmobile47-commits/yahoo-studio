...
declare global {
  interface Window {
    ...
    __YahooStudioOsmdLoader__?: Promise<OpenSheetMusicDisplayCtor>;
  }
}
...
export function loadOsmdConstructor(): Promise<OpenSheetMusicDisplayCtor> {
  ...
  if (window.__YahooStudioOsmdLoader__) {
    return window.__YahooStudioOsmdLoader__;
  }

  window.__YahooStudioOsmdLoader__ = new Promise<OpenSheetMusicDisplayCtor>((resolve, reject) => {
