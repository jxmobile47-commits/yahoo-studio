/**
 * CORS Error Suppression Utility for Yahoo Studio
 * 
...
const NOISY_DEV_WARNING_PATTERNS = [
  ...
  // Backend unavailable errors (development without Python backend)
  /Error in beat detection/i,
  /Error in chord recognition/i,
  /Error in audio processing/i,
  /Failed to detect audio duration/i,
  /Failed to estimate duration from file size/i,
  /Fallback duration estimation also failed/i,
  /Backend service unavailable/i,
  /Beat detection failed/i,
  /Chord recognition failed/i,
  /Backend returned 403 Forbidden/i,
  /Error in offload/i,
  /Port conflict: Port 5000/i,
  /Manual lyrics search failed/i,
  /Failed to detect key/i,
  /Failed to load resource.*localhost:5000/i,
  /Failed to load resource.*localhost:8000/i,
  // Firebase / Auth warnings (development without config)
  /Missing required Firebase configuration/i,
  /Firebase will not be initialized/i,
  /Firebase Auth not initialized/i,
  /Firebase not initialized/i,
  /Failed to auto-initialize Firebase/i,
  /Error initializing Firebase/i,
  /Failed to ensure authentication/i,
  /Auth state setup timeout/i,
  /Anonymous sign-in attempt.*failed/i,
  /All authentication attempts failed/i,
  /Firebase App Check initialisation failed/i,
  /Failed to obtain App Check token/i,
  /NEXT_PUBLIC_RECAPTCHA_V3_SITE_KEY not set/i,
];
